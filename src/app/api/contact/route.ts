...
export async function POST(request: NextRequest) {
  try {
    ...
    const FORM_ID = process.env.DHATU_PAY_CF7_FORM || '265';
    const WORDPRESS_API = process.env.DHATU_PAY_WORDPRESS_API;

    if (!WORDPRESS_API) {
      console.error('Missing DHATU_PAY_WORDPRESS_API environment variable');
      return NextResponse.json(
        {
          success: false,
